...
env_bias_table <- env_bias %>%
  ...
  select(Variable = variable, `D Statistic` = D_statistic,
...
trend_table <- trend_tests %>%
  ...
  select(Variable = variable, `Kendall's τ` = tau, `P-value` = p_value, Trend = trend)
...
rarity_table <- rarity_summary %>%
  ...
  select(`Rarity Class` = rarity_class, `Species` = n_species,
...
coef_table <- count_model_summary %>%
  ...
  select(Predictor = term, Estimate = estimate, `Std. Error` = std.error,
