...
kable(summary_table, caption = "Table 1: Summary statistics of GBIF data for Kenya. 'Analyzed Records' refers to the dataset after moderate quality filtering (valid coordinates, species identification, dates, and no coordinate errors), which is used for primary analyses in this manuscript. All taxonomic and temporal statistics are calculated from this filtered dataset.",
      booktabs = TRUE) %>%
...
# Format the sensitivity analysis table
sensitivity_table <- sensitivity_metrics %>%
  mutate(
    Records = format(Records, big.mark = ","),
    Retention_Pct = sprintf("%.1f%%", Retention_Pct),
    Species = format(Species, big.mark = ","),
    Families = format(Families, big.mark = ","),
    Classes = format(Classes, big.mark = ",")
  )

kable(sensitivity_table,
      col.names = c("Filtering Strategy", "Records", "Retention (%)",
                   "Species", "Families", "Classes"),
      caption = "Table 5: Sensitivity analysis comparing biodiversity metrics across three filtering strategies. The moderate strategy (used for primary analyses) balances data quality with sample size retention.",
      booktabs = TRUE,
      align = c("l", "r", "r", "r", "r", "r")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = FALSE) %>%
  row_spec(2, bold = TRUE, background = "#E8F4F8")  # Highlight the primary strategy
```
...
# Expected predictors based on model specification
expected_predictors <- c("elevation_scaled", "temperature_scaled", "precipitation_scaled",
                        "dist_to_city_scaled", "dist_from_equator", "dist_from_coast_km")
actual_predictors <- count_model_summary$term
missing_predictors <- setdiff(expected_predictors, actual_predictors)

if (length(missing_predictors) > 0) {
  cat(sprintf("\nWARNING: Missing %d expected predictor(s):\n", length(missing_predictors)))
  cat(paste("  -", missing_predictors, collapse = "\n"))
  cat("\n")
}
```
