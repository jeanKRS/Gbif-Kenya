...
**Results:** Data quality filtering removed `r format(quality_assessment$n_original - quality_assessment$n_final, big.mark=",")` records (`r round(quality_metrics$removal_rate, 1)`%), with the final cleaned dataset containing `r format(quality_assessment$n_final, big.mark=",")` records. The most common category of quality issues involved coordinate-related problems, with `r format(quality_metrics$coordinate_quality$total_coord_flags, big.mark=",")` records flagged by CoordinateCleaner tests, representing `r round(quality_metrics$coordinate_quality$percent_coord_flags, 1)`% of records evaluated at the coordinate validation stage. Beyond quality issues, our bias analyses revealed significant spatial, temporal, and taxonomic biases. Sampling effort was concentrated near major cities and roads, with strong positive spatial autocorrelation (Moran's I = `r round(moran_results$morans_i, 3)`, p < 0.001). Temporal analysis showed increasing trends in data collection, with `r round(temporal_stats$cv_records, 2)` coefficient of variation in annual records. Taxonomic coverage was highly uneven, with a Gini coefficient of `r round(gini_records, 3)` for records across classes.
