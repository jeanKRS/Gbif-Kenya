...
**Results:** Data quality filtering removed `r format(quality_assessment$n_original - quality_assessment$n_final, big.mark=",")` records (`r round(quality_metrics$removal_rate, 1)`%), with the final cleaned dataset containing `r format(quality_assessment$n_final, big.mark=",")` records. The most common category of quality issues involved coordinate-related problems, with `r format(quality_metrics$coordinate_quality$total_coord_flags, big.mark=",")` records flagged by CoordinateCleaner tests, representing `r round(quality_metrics$coordinate_quality$percent_coord_flags, 1)`% of the original dataset. Beyond quality issues, our bias analyses revealed significant spatial, temporal, and taxonomic biases. Sampling effort was concentrated near major cities and roads, with strong positive spatial autocorrelation (Moran's I = `r round(moran_results$morans_i, 3)`, p < 0.001). Temporal analysis showed increasing trends in data collection, with `r round(temporal_stats$cv_records, 2)` coefficient of variation in annual records. Taxonomic coverage was highly uneven, with a Gini coefficient of `r round(gini_records, 3)` for records across classes.
...
- How many records were removed at each filtering step
- What specific quality issues were most prevalent
...
- Assess the reliability of cleaned datasets
- Compare data quality across regions or taxonomic groups
- Understand whether quality standards were appropriate for the research question
...
By combining these tools with transparent documentation of quality filtering steps, we can address the dual challenges of quality and bias in a systematic, reproducible way. Understanding both aspects is critical for:
...
This study provides the first comprehensive, reproducible assessment of both data quality issues and systematic biases in biodiversity data for Kenya using standardized methods. Unlike previous studies that focus on either quality or bias, we integrate both perspectives to provide a complete picture of data reliability and representativeness.

Our specific objectives are to:

1. **Systematically document all data quality issues** encountered during cleaning, reporting the number and percentage of records affected by each specific issue (missing coordinates, high uncertainty, missing taxonomy, temporal problems, duplicates, coordinate flags)

2. **Quantify spatial bias patterns** across Kenya using grid-based analyses, assess sampling completeness, and test for spatial autocorrelation to identify clustered vs. dispersed sampling

3. **Evaluate environmental bias** by comparing sampled locations to available environmental space, determining whether certain habitats, elevations, or climate zones are over- or under-represented

4. **Assess temporal patterns** in data collection, identify temporal gaps and trends, and quantify temporal completeness to understand how data availability varies over time

5. **Evaluate taxonomic bias** across major taxonomic groups using inequality metrics (Gini coefficients), diversity indices, and rarity analyses to identify which taxa are well-sampled vs. neglected

6. **Identify environmental and geographic predictors** of sampling effort using statistical models (GLMs and GAMs) to understand what drives sampling patterns (accessibility, urbanization, environmental conditions)

7. **Provide actionable recommendations** for targeted sampling to address identified gaps, enabling more strategic future data collection

8. **Present a reproducible framework** for comprehensive data quality and bias assessment that can be readily applied to other countries, regions, or taxonomic groups

By achieving these objectives, we provide both a thorough assessment of Kenya's biodiversity data and a methodological template for transparent, reproducible data quality and bias assessment in biodiversity informatics.
...
Data cleaning followed a sequential filtering approach where each step removes specific categories of problematic records. The order of filtering steps matters: we first remove records with fundamental issues (e.g., missing coordinates) before applying more sophisticated tests (e.g., coordinate quality flags) to the remaining data. This sequential approach:

1. **Reduces computational burden**: Removing obviously bad records early speeds up later analyses
2. **Prevents cascading errors**: Checking taxonomic completeness before coordinate quality avoids wasting time validating coordinates for records we'll exclude anyway
3. **Enables precise tracking**: Each step's impact can be quantified independently
4. **Facilitates sensitivity analyses**: Users can assess how different quality standards affect results

The specific cleaning steps applied were:

- **Removal of records with missing or invalid coordinates**: Essential for any spatial analysis; records without coordinates cannot be mapped or analyzed spatially
- **Removal of records with coordinate uncertainty >10 km**: High uncertainty makes records unsuitable for fine-scale analyses. The 10 km threshold balances data retention with spatial precision requirements for most conservation applications
- **Exclusion of fossil and living specimens**: These represent historical species occurrences or captive individuals, not current wild populations relevant to contemporary biodiversity assessment
- **Removal of records without species-level identification**: Species is the fundamental unit for most biodiversity analyses. Records identified only to genus or family cannot contribute to species-specific questions
- **Temporal filtering (1950-present)**: Focuses on recent biodiversity patterns while excluding historical records that often lack reliable georeferencing and may not reflect current distributions due to land use change
- **Duplicate record removal**: The same observation may appear multiple times due to data aggregation from multiple sources, inflating species counts and sampling effort
- **Coordinate cleaning using CoordinateCleaner**: Automated detection of common georeferencing errors such as swapped coordinates, institutional locations, country centroids, and statistical outliers [@zizka2019]

### Cleaned Dataset Summary

The final cleaned dataset contained **`r format(nrow(kenya_data), big.mark=",")`** occurrence records representing **`r format(summary_stats$n_species, big.mark=",")`** species from **`r format(summary_stats$n_families, big.mark=",")`** families across **`r format(summary_stats$n_classes, big.mark=",")`** classes. This represents a substantial biodiversity knowledge base, though as we show in subsequent analyses, it exhibits significant quality issues and systematic biases that must be considered when using these data.
...
**Important Note:** Each test is independent, so a single record can be flagged by multiple tests. The final quality metric counts unique records flagged by ANY test, not the sum of individual test flags. Records flagged by any test were removed from the final dataset to ensure high coordinate quality, though researchers with different requirements could apply less stringent criteria.
...
**Interpretation**: The `r round(removal_rate, 1)`% removal rate represents a substantial quality filtering effort, reflecting the high prevalence of issues in aggregated biodiversity data. This is comparable to quality filtering rates reported in other regional GBIF assessments (typically 40-95% removal rates depending on quality standards). The high removal rate underscores that GBIF data, while invaluable, require rigorous quality control. Data users must document and report their quality filtering decisions to ensure transparency and enable appropriate interpretation of results. Different applications may require different quality standards - for example, broad-scale macroecological studies might accept higher coordinate uncertainty than fine-scale conservation planning.
...
# First, get all non-coordinate steps
quality_table_base <- quality_tracking %>%
  filter(step != "Coordinate flags")

# Get the coordinate flags row to use for records_remaining
coord_flags_row <- quality_tracking %>%
  filter(step == "Coordinate flags")

# Create individual rows for each coordinate test
coord_details <- coord_issues %>%
  filter(records_flagged > 0) %>%
  arrange(desc(records_flagged)) %>%
  mutate(
    step = paste0("  • ", test),
    description = description,
    records_removed = records_flagged,
    # Use the same records_remaining for all (since they're independent tests)
    records_remaining = coord_flags_row$records_remaining,
    percent_removed = percent_of_original,
    cumulative_percent_removed = coord_flags_row$cumulative_percent_removed
  ) %>%
  select(step, description, records_removed, records_remaining,
         percent_removed, cumulative_percent_removed)

# Add a header row for coordinate tests
coord_header <- tibble(
  step = "Coordinate flags (details below)",
  description = paste0("Records flagged by ANY CoordinateCleaner test (",
                       format(coord_flags_row$records_removed, big.mark = ","),
                       " unique records)"),
  records_removed = coord_flags_row$records_removed,
  records_remaining = coord_flags_row$records_remaining,
  percent_removed = coord_flags_row$percent_removed,
  cumulative_percent_removed = coord_flags_row$cumulative_percent_removed
)

# Combine all rows
quality_table_expanded <- bind_rows(
  quality_table_base,
  coord_header,
  coord_details
) %>%
...
kable(quality_table_expanded,
      caption = "Detailed breakdown of data quality filtering steps. Each row shows the number and percentage of records removed at each sequential filtering step. Coordinate flags are expanded to show individual test results. Note: Individual coordinate tests may overlap (one record can be flagged by multiple tests), so the sum of individual test counts exceeds the unique records removed.",
      booktabs = TRUE,
      align = c("l", "l", "r", "r", "r", "r")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = TRUE,
