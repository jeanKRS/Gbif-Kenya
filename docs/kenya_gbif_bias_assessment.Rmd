...
# Display data limitation notice if predictors are missing
cat('<div style="
  ...
  border-radius: 5px;
">')
cat('<strong>⚠️ Data Availability Note:</strong><br>')
cat('The fitted statistical model includes <strong>', n_count_predictors, ' of ', n_expected, ' expected predictors</strong> due to data availability constraints.<br><br>')
cat('<strong>Available predictors:</strong> ', paste(gsub("_scaled", "", count_model_predictors), collapse = ", "), '<br>')
if(length(missing_count) > 0) {
  cat('<strong>Unavailable predictors:</strong> ', paste(gsub("_scaled|_km", "", missing_count), collapse = ", "), '<br><br>')
  cat('<em>Analysis Implications:</em> The model focuses on environmental predictors (elevation, temperature, precipitation) ')
  cat('and may not capture accessibility-related biases (distance to cities, coast) or latitudinal gradients. ')
  cat('This represents a common constraint in biodiversity data analysis where complete covariate coverage is not always available. ')
  cat('Results should be interpreted as preliminary environmental bias assessment rather than comprehensive sampling effort prediction.')
}
cat('</div>')
```
...
The modeling approach initially attempted to fit a GLM with six environmental and geographic predictors (elevation, temperature, precipitation, distance to nearest city, distance from equator, and distance from coast). However, due to model convergence issues, the final fitted *`r modeling_summary$model_type`* model includes three core environmental predictors: elevation, temperature, and precipitation (see Methods section). <a id="table-12"></a>[**Table 12**](#table-12) presents the coefficient estimates for all predictors included in the fitted model.
...
Under-sampled areas (red) represent priority targets for future sampling efforts, as they are under-represented even after accounting for their environmental characteristics.
...
if (file.exists(spatial_grid_file) && file.exists(grid_pred_file)) {
  ...
  pal_residual <- colorNumeric(
    palette = colorRampPalette(c("#B2182B", "grey", "#2166AC"))(1000),
    domain = c(-rng, rng),
    na.color = "transparent")
