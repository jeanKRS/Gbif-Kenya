...
2. **Trace the pipeline**: Follow the data flow from raw data → processing → analysis → visualization
...
Raw GBIF Data
    ↓
01_data_download.R → kenya_gbif_raw.rds
    ↓
01b_data_quality_assessment.R → kenya_gbif_flagged.rds
    ↓
┌───────────┬────────────┬───────────────┬─────────────────┐
│           │            │               │                 │
...
- Kendall's τ ranges from -1 (strong negative trend) to +1 (strong positive trend)
...
- Treemaps show hierarchical structure (kingdom → phylum → class)
...
**Renders as**:
```
🔍 View temporal aggregation code (click to expand)
...
1. **Start with Table 1** (Summary Statistics)
   ...
   - Understand: Raw data → Quality filtering → Summary
...
- **Kendall's τ**: Ranges from -1 (decreasing) to +1 (increasing)
...
4. **Environmental Bias**: Reddy & Dávalos 2003. Geographical sampling bias and its implications for conservation priorities in Africa. *Journal of Biogeography* 30(11):1719-1727.
